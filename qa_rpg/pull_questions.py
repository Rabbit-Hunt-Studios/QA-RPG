...
sys.path.insert(0, '') # Django project path
...
for question in all_question['results']:
    ...
    question_in_db = Question.objects.filter(owner=admin).values_list('question_text', flat=True)
    if question_text not in question_in_db:
